...
/**
 * Truth-Based Eprolo Bridge (v4.1)
 * Standardized response schema for deterministic UI interpretation.
 * Includes MD5 Signature Engine for Eprolo API Authentication.
 */

// 🔒 LIGHTWEIGHT MD5 ENGINE (Standard Implementation)
function md5(string) {
    ...
    function md51(s) {
        ...
        tail[14] = n * 8; md5_Cycle(state, tail); return state;
    }
    function md5blk(s) {
        var p = [], i;
        for (i = 0; i < 64; i += 4) p[i >> 2] = s.charCodeAt(i) + (s.charCodeAt(i + 1) << 8) + (s.charCodeAt(i + 2) << 16) + (s.charCodeAt(i + 3) << 24);
        return p;
    }
...
class EproloService {
    ...
    async searchProducts(query, page = 0) {
        ...
        const signRaw = `${this.apiKey}${timestamp}${this.apiSecret}`;
        const signature = md5(signRaw);
        ...
        const debugInfo = {
            ...
            httpStatus: null,
            rawResponse: null,
            authUsed: { apiKey: this.apiKey ? 'PRESENT' : 'MISSING', sign: signature ? 'GENERATED' : 'NULL' }
        };
